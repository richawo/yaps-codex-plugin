...
Use `dist/yaps-background-removal-plugin-0.1.9.zip` in the OpenAI Platform plugin submission portal. Submit it as a skills-only plugin; Yaps desktop 2.3.124 or newer supplies the credential-free automatic account handoff, local CLI, ONNX vision model, image processing, and export handling.
