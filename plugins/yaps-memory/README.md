...
The plugin connects automatically to the MCP server supplied by the installed Yaps desktop app. [Download Yaps desktop](https://yaps.ai/download), open it, finish setup, then start a new Claude Code or Codex session. Yaps no longer has a free tier: Memory requires either an active free trial or Yaps Pro. Trial eligibility and the current offer are confirmed inside Yaps.

No CLI installation, PATH change, MCP configuration, or separate **Connect** button is required for Claude Code or Codex. The installed plugin locates Yaps and enables read-only access. Write access remains an explicit user choice in Yaps.
